...
export default function App() {
  ...
  const [notifications] = useState([
    {
      ...
      message: "Your Rent/Mortgage is currently 1 day past due.",
      time: "2h ago",
    ...
    },
    {
      ...
      message: "$150.00 was successfully moved to your High-Yield Savings.",
      time: "1d ago",
  ...
  const [todos, setTodos] = useState([
    {
      id: 1,
      text: "Call insurance about policy update",
      priority: 5,
      type: "task",
      isCompleted: false,
    },
    {
      id: 2,
      text: "Cancel unused streaming service",
      priority: 3,
      type: "task",
      isCompleted: false,
    },
    {
      id: 3,
      text: "Move funds to High-Yield Savings",
      priority: 4,
      type: "task",
      isCompleted: true,
    },
  ...
  // === MOCK DATA (PHASE 1 - THE MEDIAN OHIO PERSONA) ===
  ...
  const strokeDashoffset =
    strokeDasharray - (strokeDasharray * safePercentage) / 100;
  ...
  const handleBillClick = (id) => {
    ...
    if (!bill.isPaid) {
      setPaymentModalConfig({
        ...
        billId: id,
        accountId:
          accounts.find((a) => a.type === "Checking" || a.type === "Cash")
            ?.id || accounts[0].id,
      });
    } else {
      const refundAccountId = bill.paidFromAccountId || 201;
  ...
  const confirmPaymentRoute = () => {
    ...
    const autoTimeStamp = `${currentTime.toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
  ...
  const handleAddAccount = () => {
    ...
    // Check if new account type is a Credit Card to assign negative balance
    const isCreditCard = newAccType === "Credit Card";
    const finalBalance = isCreditCard
      ? -Math.abs(startBal)
      : Math.abs(startBal);

    // Assign specific icons based on the new account types
    const getIcon = (type) => {
  ...
  const executeTransfer = () => {
    ...
    setAccounts(
      accounts.map((acc) => {
        if (acc.id === parseInt(transferFrom))
          return { ...acc, balance: acc.balance - amt };
        if (acc.id === parseInt(transferTo))
          return { ...acc, balance: acc.balance + amt };
        return acc;
      })
    );
  ...
  const updateAccountBalance = () => {
    ...
    // Check if updated account is Credit Card to maintain negative state
    const isCreditCard = selectedAccount.type === "Credit Card";
    const finalBalance = isCreditCard ? -Math.abs(newBal) : Math.abs(newBal);

    setAccounts(
      accounts.map((acc) =>
        acc.id === selectedAccount.id ? { ...acc, balance: finalBalance } : acc
      )
    );
    ...
    if (btn === "=") {
      try {
        ...
        if (/^[0-9+\-*/. ]+$/.test(toEval)) {
          ...
          setInputValue(
            String(Function('"use strict";return (' + toEval + ")")())
          );
        }
      } catch (e) {
        setInputValue("Error");
  ...
  const handleConfirmAction = () => {
    ...
    if (drawerTab === "bills") {
      ...
      const newBill = {
        id: Date.now(),
        name: entryName || "New Bill",
        icon: entryIcon || "📋",
        amount: amountToProcess,
        date: sortableDay,
        fullDate: displayDate,
        payday: assignedPayday,
        isPaid: false,
        isOverdue: false,
      ...
      const targetAccId = entryAccount
        ? parseInt(entryAccount)
        : accounts.find((a) => a.type === "Checking" || a.type === "Cash")
            ?.id || accounts[0].id;
      setTransactions([
        {
          id: Date.now(),
          name: entryName || "Income Deposit",
          icon: "💵",
          amount: amountToProcess,
          date: autoTimeStamp,
          type: "Income",
      ...
      setAccounts(
        accounts.map((acc) =>
          acc.id === targetAccId
            ? { ...acc, balance: acc.balance + amountToProcess }
            : acc
        )
      );
  ...
  const renderHeroShell = (title, graphicContent) => (
    <header
      className={`px-6 pt-12 pb-5 rounded-b-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden transition-colors duration-500 mb-8 ${
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8 relative z-10 h-10">
        <button
          ...
          className={`w-10 h-10 rounded-full flex items-center justify-center border transition-colors shadow-sm ${
            isDarkMode
              ? "bg-slate-800 border-slate-700 text-slate-300 hover:text-[#1877F2]"
              : "bg-white border-slate-100 text-slate-400 hover:text-[#1877F2]"
          }`}
        >
          {isDarkMode ? <Sun size={18} /> : <Moon size={18} />}
        </button>
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center -top-1">
          <span className="text-[11px] font-black text-[#1877F2] uppercase tracking-[0.2em] leading-none mb-0.5">
            Ledger
          </span>
          <span className="text-[16px] font-black text-[#1877F2] uppercase tracking-[0.15em] leading-none">
            Planner
          </span>
        </div>
        <button
          onClick={() => setIsNotificationsOpen(true)}
          className={`relative w-10 h-10 rounded-full flex items-center justify-center border transition-colors shadow-sm ${
            isDarkMode
              ? "bg-slate-800 border-slate-700 text-slate-300 hover:text-[#1877F2]"
              : "bg-white border-slate-100 text-slate-400 hover:text-[#1877F2]"
          }`}
        >
          <Bell size={18} />
          <span
            className={`absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 ${
      ...
      <h2
        className={`text-3xl font-black tracking-tight mb-6 relative z-10 ${
      ...
      {graphicContent}
      <div
        className={`relative z-10 pt-4 border-t flex justify-between items-center ${
      ...
      >
        ...
        <span className="text-[9px] font-black uppercase tracking-wider text-[#1877F2]">
          {formattedTime}
        </span>
      </div>
    </header>
        ...
        <div className="w-full max-w-md bg-white h-full relative shadow-2xl flex flex-col px-8 pt-24 pb-10">
          <div className="flex flex-col items-center mb-12">
            <div className="w-24 h-24 rounded-full flex items-center justify-center mb-6 shadow-xl border-4 border-white dark:border-slate-800 overflow-hidden bg-white">
              <img
                src="/login-logo.png"
                alt="Ledger Planner Logo"
                className="w-full h-full object-cover"
              />
            </div>

            <div className="text-center mb-2">
              <span className="text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] block leading-none mb-1">
                Ledger
              </span>
              <span className="text-xl font-black text-[#1877F2] uppercase tracking-[0.15em] block leading-tight">
                Planner
              </span>
            </div>
          ...
          <form onSubmit={handleFakeLogin} className="space-y-5 flex-1">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm font-bold rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-[#1877F2] focus:ring-1 focus:ring-[#1877F2] transition-all"
                  placeholder="ledgerplanner@gmail.com"
                />
                <Mail
            ...
            <button
              ...
              className={`w-full mt-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
                isLoading || !email || !password
                  ? "bg-slate-200 text-slate-400 cursor-not-allowed"
                  : "bg-[#1877F2] text-white hover:bg-blue-600 active:scale-[0.98] shadow-[0_8px_20px_rgba(24,119,242,0.3)]"
              }`}
            >
              {isLoading ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
                "Initialize Ledger Planner"
              )}
            </button>
          </form>
        </div>
  ...
  const renderHome = () => {
    ...
    bills.forEach((bill) => {
      if (billsByPayday[bill.payday]) billsByPayday[bill.payday].push(bill);
    });
    ...
    const graphicContent = (
      <div className="flex items-center justify-between relative z-10 mb-6">
        <div className="relative w-36 h-36 flex-shrink-0">
          ...
          >
            ...
            <circle
              ...
              r="40"
              fill="transparent"
              stroke={isNegative ? "url(#redGlow)" : "url(#blueGlow)"}
              strokeWidth="12"
              strokeLinecap="round"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span
              className={`text-2xl font-black ${
                isNegative ? "text-red-500" : "text-[#1877F2]"
              }`}
            >
        ...
        <div className="flex-1 pl-6 text-right">
          ...
          >
            <div
              className={`w-1.5 h-1.5 rounded-full ${
                isNegative ? "bg-red-500" : "bg-emerald-500 animate-pulse"
              }`}
            ></div>
          ...
          >
            $
            {aaronsBalance.toLocaleString("en-US", {
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-end gap-2 text-xs font-bold uppercase">
              ...
              <span
                className={
                  isDarkMode
                    ? "text-emerald-400 bg-emerald-900/30 px-2 rounded"
                    : "text-emerald-600 bg-emerald-50 px-2 rounded"
                }
              >
                $
                {totalIncomeBalance.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                })}
              </span>
            </div>
            <div className="flex justify-end gap-2 text-xs font-bold uppercase">
              ...
              <span
                className={
                  isDarkMode
                    ? "text-slate-300 bg-slate-800 px-2 rounded"
                    : "text-slate-700 bg-slate-100 px-2 rounded"
                }
              >
                $
                {unpaidBillsAmount.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                })}
              </span>
            </div>
    ...
    return (
      <div
        className={`animate-fade-in pb-32 transition-colors duration-500 ${
        ...
        <main className="px-6 space-y-4">
          <div className="flex justify-between items-center px-1 mt-2">
            ...
            <button
              ...
              className={`text-[9px] font-black uppercase flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors ${
                isDarkMode
                  ? "bg-slate-800 text-[#1877F2] hover:bg-slate-700"
          ...
          <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-4 pt-2 -mx-2 px-3 snap-x">
            {["Payday 1", "Payday 2", "Payday 3", "Payday 4", "Payday 5"].map(
              (pd) => {
                const pdSettings = paydayConfig[pd];
                const isSet =
                  pdSettings && (pdSettings.date || pdSettings.income);
                return (
                  <div
                    key={pd}
                  ...
                  >
                    <p
                      className={`text-[9px] font-black uppercase tracking-widest mb-2 ${
                        isSet ? "text-[#1877F2]" : "text-slate-400"
                      }`}
                    >
                      {pd}
                    </p>
                    ...
                    >
                      {pdSettings?.income
                        ? `$${parseFloat(pdSettings.income).toLocaleString()}`
                        : "$0.00"}
                    </p>
                    <p
          ...
          <div className="space-y-4">
            {Object.entries(billsByPayday).map(([payday, groupBills]) => {
              ...
              const checkTotal = groupBills
                .filter((b) => !b.isPaid)
                .reduce((sum, b) => sum + b.amount, 0);
              const pdSettings = paydayConfig[payday];
              const expectedDateStr = isDueNow
                ? "Currently Due"
                : pdSettings?.date
                ? formatPaydayDateStr(pdSettings.date)
                : "Unscheduled";
              const expectedIncomeStr =
                pdSettings && pdSettings.income
                  ? `+$${parseFloat(
                      pdSettings.income
              ...
              return (
                <div key={payday} className="space-y-2">
                  ...
                  {!isCollapsed && (
                    <div
                      className={`rounded-[2rem] p-3 border ${
                        isDueNow
                          ? isDarkMode
                    ...
                    >
                      {sortedBills.length === 0 ? (
                        ...
                        sortedBills.map((bill, idx) => (
                          <div
                            key={bill.id}
                            className={`flex flex-col p-3 rounded-2xl transition-all duration-300 ${
                              isDarkMode
                                ? "hover:bg-slate-800/50"
                                : "hover:bg-slate-50/50"
                            } ${idx !== sortedBills.length - 1 ? "mb-1" : ""}`}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4 flex-1">
                                <div
                                  className="relative p-1 z-10 cursor-pointer"
                            ...
                            {bill.isInstallment && (
                              <div className="mt-2.5 ml-[60px] pr-2">
                                <div className="flex justify-between text-[8px] font-black uppercase tracking-wider mb-1 text-slate-400">
                                  ...
                                  <span>
                                    ${bill.totalAmount.toLocaleString()} Total
                                  </span>
                                </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800 mt-8">
            ...
            <div
              className={`rounded-[2rem] p-3 border shadow-sm ${
                isDarkMode
                  ? "bg-[#1E293B] border-slate-800"
                  : "bg-white border-slate-50"
              }`}
            >
              {transactions.slice(0, 5).map((tx, idx) => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-2xl cursor-pointer transition-colors ${
                    ...
                    idx !== Math.min(transactions.length, 5) - 1 ? "mb-1" : ""
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-11 h-11 rounded-2xl border flex items-center justify-center text-xl ${
                        isDarkMode
                          ? "bg-[#0F172A] border-slate-700"
                          : "bg-white border-slate-100"
                      }`}
              ...
              <button
                ...
                className={`w-full mt-4 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-[0.98] ${
                  isDarkMode
                    ? "bg-[#1877F2] text-white shadow-blue-900/20"
                    : "bg-[#1877F2] text-white shadow-blue-500/30"
                }`}
              >
      ...
      <div className="relative z-10 mb-2">
        <div className="flex justify-between items-end mb-6">
          <div>
            ...
            >
              $
              {activeDataPoint.val.toLocaleString("en-US", {
                minimumFractionDigits: 2,
              })}
            </p>
          </div>
          ...
          {historyData.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="w-full relative flex justify-center h-full items-end">
                  <div
                    className={`w-full max-w-[32px] rounded-t-xl transition-all duration-500 ease-out ${
                      isActive
                        ? "bg-[#1877F2] shadow-[0_0_15px_rgba(24,119,242,0.4)]"
                        : isDarkMode
                ...
                <span
                  className={`text-[9px] font-black mt-3 uppercase tracking-wider transition-colors duration-300 ${
                    isActive ? "text-[#1877F2]" : "text-slate-400"
                  }`}
                >
                  {item.label}
                </span>
      ...
      >
        ...
        <main className="px-6 space-y-6">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-[0.98] ${
              isDarkMode
                ? "bg-[#1877F2] text-white shadow-blue-900/20"
                : "bg-[#1877F2] text-white shadow-blue-500/30"
            }`}
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 border transition-all active:scale-[0.98] ${
              isDarkMode
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      setEditAccountBalance(Math.abs(acc.balance).toString());
                    }}
                    className={`flex items-center justify-between p-3 rounded-2xl cursor-pointer transition-colors ${
                      isDarkMode
                        ? "hover:bg-slate-800/50"
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-2xl border flex items-center justify-center text-xl bg-opacity-10 ${
                          isDarkMode
  ...
  const renderBills = () => {
    ...
    const progressPercentage =
      Math.round((paidBillsAmount / totalBillsAmount) * 100) || 0;
      ...
      <div className="flex items-center justify-between relative z-10 mb-6">
        ...
        <div className="flex-1 pl-8 text-right">
          ...
          <p className="text-xs font-bold text-slate-400">
            <span className="text-[#10B981]">
              $
              {paidBillsAmount.toLocaleString("en-US", {
                minimumFractionDigits: 2,
              })}
            </span>{" "}
            paid of ${totalBillsAmount.toLocaleString()}
          </p>
        </div>
      </div>
    );
        ...
        <main className="px-6 space-y-8">
          <div className="space-y-4 mb-8">
            ...
            {unpaidBills.length === 0 ? (
              <div
                className={`p-8 text-center rounded-[2rem] border border-dashed ${
                  isDarkMode
            ...
            <div className="space-y-4">
              ...
              >
                {paidBills.map((bill, idx) => (
                  <div
                    key={bill.id}
                    className={`flex flex-col p-3 rounded-2xl transition-all duration-300 opacity-60 grayscale-[0.3] ${
                      isDarkMode
                        ? "hover:bg-slate-800/50 hover:opacity-100"
                        : "hover:bg-slate-50/50 hover:opacity-100"
                    } ${idx !== paidBills.length - 1 ? "mb-1" : ""}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        <div
  ...
  const renderActivity = () => {
    const filteredTxs = transactions.filter((tx) => {
      const matchesSearch =
        tx.name.toLowerCase().includes(activitySearch.toLowerCase()) ||
        tx.category.toLowerCase().includes(activitySearch.toLowerCase());
      const matchesFilter =
    ...
    const incomePct =
      totalActivity === 0 ? 50 : (totalIncome / totalActivity) * 100;
      ...
      <div className="flex flex-col relative z-10 mb-6">
        <div className="text-center mb-6">
          ...
          <p
            className={`text-5xl font-black tracking-tighter ${
              netFlow >= 0 ? "text-emerald-500" : "text-orange-500"
            }`}
          >
            {netFlow >= 0 ? "+" : "-"}$
        ...
        <div className="flex w-full h-10 rounded-2xl overflow-hidden shadow-inner bg-slate-100 dark:bg-slate-800">
          <div
            ...
            className="bg-[#10B981] flex items-center pl-4 transition-all duration-1000"
          >
            <span className="text-[10px] font-black text-white tracking-widest uppercase shadow-sm">
              In
            </span>
          </div>
          <div
            style={{ width: `${100 - incomePct}%` }}
            className="bg-[#F97316] flex items-center justify-end pr-4 transition-all duration-1000"
          >
            <span className="text-[10px] font-black text-white tracking-widest uppercase shadow-sm">
              Out
            </span>
        ...
        <main className="px-6 space-y-6">
          <div className="relative shadow-sm">
            <input
              ...
              className={`w-full py-4 pl-12 pr-4 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                isDarkMode
                  ? "bg-[#1E293B] border-slate-700 text-white placeholder-slate-500 focus:border-[#1877F2]"
                  : "bg-white border-slate-100 text-slate-900 placeholder-slate-400 focus:border-[#1877F2]"
              }`}
            />
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"
          ...
          <div className="flex gap-2">
            {["All", "Income", "Expense"].map((filterOption) => (
              <button
                ...
                className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${
                  activityFilter === filterOption
                    ? "bg-[#1877F2] text-white shadow-[0_4px_15px_rgba(24,119,242,0.3)]"
                    : isDarkMode
  ...
  const renderTodo = () => {
    ...
    const activeTasks = todos
      .filter((t) => !t.isCompleted && t.type === "task")
      .sort((a, b) => b.priority - a.priority);
    const activeShopping = todos
      .filter((t) => !t.isCompleted && t.type === "shopping")
      .sort((a, b) => b.priority - a.priority);
        ...
        <main className="px-6 space-y-8">
          ...
          >
            <input
              ...
              className={`w-full py-3 px-4 mb-4 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                isDarkMode
                  ? "bg-[#0F172A] border-slate-700 text-white placeholder-slate-500 focus:border-[#1877F2]"
                  : "bg-slate-50 border-slate-100 text-slate-900 placeholder-slate-400 focus:border-[#1877F2]"
              }`}
            />
            <div className="flex justify-between items-center">
              <div className="flex flex-col gap-2">
                <div className="flex gap-1 bg-slate-50 dark:bg-[#0F172A] p-1 rounded-xl border dark:border-slate-700 border-slate-100">
                  <button
                    type="button"
                    onClick={() => setNewTodoType("task")}
                    className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${
                      newTodoType === "task"
                        ? "bg-white dark:bg-slate-700 shadow-sm text-[#1877F2]"
                        : "text-slate-400"
                    }`}
                  >
                    To-Do
                  ...
                  <button
                    type="button"
                    onClick={() => setNewTodoType("shopping")}
                    className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${
                      newTodoType === "shopping"
              ...
              <button
                ...
                className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-transform active:scale-95 ${
                  !newTodoText.trim()
                    ...
                    : "bg-[#1877F2] text-white shadow-[0_8px_20px_rgba(24,119,242,0.3)]"
                }`}
              >
          ...
          <div className="space-y-4">
            ...
            {activeTasks.length === 0 ? (
              ...
              >
                <CheckSquare
                  size={32}
                  className="mx-auto mb-3 text-[#1877F2] opacity-50"
                />
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-2xl transition-all duration-300 ${
                      isDarkMode
                        ? "hover:bg-slate-800/50"
                        : "hover:bg-slate-50/50"
                    } ${idx !== activeShopping.length - 1 ? "mb-1" : ""}`}
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div
                        className="relative p-1 z-10 cursor-pointer"
            ...
            <div className="space-y-4">
              ...
              >
                {completedTodos.map((todo, idx) => (
                  <div
                    key={todo.id}
                    className={`flex items-center justify-between p-3 rounded-2xl transition-all duration-300 opacity-60 grayscale-[0.3] ${
                      isDarkMode
                        ? "hover:bg-slate-800/50 hover:opacity-100"
                        : "hover:bg-slate-50/50 hover:opacity-100"
                    } ${idx !== completedTodos.length - 1 ? "mb-1" : ""}`}
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div
                        className="relative p-1 z-10 cursor-pointer"
  ...
  return (
    <div
      className={`h-screen font-sans relative overflow-hidden flex justify-center transition-colors duration-500 ${
        isDarkMode ? "bg-[#0F172A]" : "bg-[#F8FAFC]"
      }`}
    >
      <div
        className={`w-full max-w-md h-full relative shadow-2xl flex flex-col transition-colors duration-500 overflow-hidden ${
          isDarkMode ? "bg-[#0F172A]" : "bg-[#F8FAFC]"
        }`}
      >
        ...
        {selectedEntry && (
          <div className="absolute inset-0 z-[60] flex items-end">
            ...
            ></div>
            <div
              className={`w-full rounded-t-[3rem] shadow-2xl animate-slide-up relative z-10 flex flex-col border-t max-h-[90vh] transition-colors duration-500 ${
                isDarkMode
                  ? "bg-[#1E293B] border-slate-700"
                  : "bg-white border-slate-100"
              }`}
                ...
                <>
                  <div className="px-8 pt-6 pb-4 overflow-y-auto hide-scrollbar">
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      <div
                        className={`w-14 h-14 rounded-2xl border flex items-center justify-center text-2xl bg-opacity-10 ${
                          isDarkMode
                            ? "bg-slate-800 border-slate-700"
                            : "bg-slate-50 border-slate-100"
                        }`}
                    ...
                    <div className="text-center mt-8 mb-2 flex justify-center items-center relative">
                      ...
                      <button
                        onClick={() =>
                          setEditEntryAmount(
                            editEntryAmount.slice(0, -1) || "0"
                          )
                        }
                        className="absolute right-4 text-slate-400 p-2 hover:text-slate-600 transition-colors"
                      >
                        ⌫
                      </button>
                    </div>
                  ...
                  <div
                    className={`p-6 mt-auto rounded-b-[3rem] shrink-0 ${
                      ...
                      {[
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (btn === "=") {
                              try {
                                const toEval = editEntryAmount
                                  .replace(/×/g, "*")
                                  .replace(/÷/g, "/");
                                if (/^[0-9+\-*/. ]+$/.test(toEval)) {
                                  // eslint-disable-next-line no-new-func
                                  const result = Function(
                                    '"use strict";return (' + toEval + ")"
                                  )();
                                  setEditEntryAmount(String(result));
                                }
                              } catch (e) {
                                setEditEntryAmount("0");
                              }
                            } else if (editEntryAmount === "0" && btn !== ".") {
                              setEditEntryAmount(btn);
                            } else {
                              setEditEntryAmount(editEntryAmount + btn);
                            }
                          }}
                          className={`h-14 rounded-2xl text-xl font-bold flex items-center justify-center transition-colors shadow-sm ${
                            ["÷", "×", "-", "+", "="].includes(btn)
                              ? isDarkMode
                ...
                <div className="px-8 pt-6 pb-12 flex flex-col h-full overflow-y-auto hide-scrollbar">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-5">
                      <div className="w-16 h-16 rounded-[2rem] bg-slate-50 dark:bg-[#0F172A] border border-slate-100 dark:border-slate-800 flex items-center justify-center text-4xl shadow-sm shrink-0">
                        {selectedEntry.icon}
                      </div>
                      <div>
                        <h2
                        ...
                        <p className="text-xs font-bold text-[#1877F2] uppercase tracking-widest">
                          {selectedEntry.category || "Recurring Bill"}
                        </p>
                      </div>
                    </div>
                    <div className="text-right mt-2 shrink-0">
                      ...
                      >
                        ${Math.abs(selectedEntry.amount).toFixed(2)}
                      </p>
                      <p className="text-[10px] font-black text-slate-400 uppercase">
                        {selectedEntry.fullDate || selectedEntry.date}
                      </p>
                    </div>
                  </div>
                  ...
                  {selectedEntry.isInstallment && (
                    <div
                      className={`mt-2 mb-6 p-4 rounded-2xl border ${
                        isDarkMode
                    ...
                    >
                      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 text-slate-400">
                        <span className="text-[#1877F2]">
                          ${selectedEntry.paidAmount.toLocaleString()} Paid
                        </span>
                        <span>
                          ${selectedEntry.totalAmount.toLocaleString()} Total
                        </span>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-auto">
                    <button
                      onClick={() => {
                        ...
                        setEditEntryAmount(
                          Math.abs(selectedEntry.amount).toString()
                        );
                      }}
                    ...
                    {selectedEntry.fullDate && (
                      <button
                        onClick={() => {
                          handleBillClick(selectedEntry.id);
                          setSelectedEntry(null);
                        }}
                        className={`py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all ${
                          selectedEntry.isPaid
                            ? "bg-slate-200 text-slate-400"
                            : "bg-[#1877F2] text-white shadow-blue-500/20"
                        }`}
                      >
                  ...
                  <button
                    onClick={() => {
                      if (selectedEntry.fullDate) {
                        setBills(
                          bills.filter((b) => b.id !== selectedEntry.id)
                        );
                      } else {
                        const tx = transactions.find(
                          (t) => t.id === selectedEntry.id
                        );
                        if (tx && tx.accountId) {
                          setAccounts(
                            accounts.map((acc) => {
                        ...
                        setTransactions(
                          transactions.filter((t) => t.id !== selectedEntry.id)
                        );
                      }
                      setSelectedEntry(null);
                    }}
        ...
        {/* 1.5 SMART PAYMENT ROUTING MODAL */}
          ...
          (() => {
            ...
            return (
              <div className="absolute inset-0 z-[80] flex items-end">
                ...
                ></div>
                <div
                  className={`w-full rounded-t-[3rem] shadow-2xl animate-slide-up relative z-10 flex flex-col max-h-[90vh] transition-colors duration-500 ${
                    isDarkMode
                      ? "bg-[#1E293B] border-slate-700"
                      : "bg-white border-slate-100"
                  }`}
                >
                  ...
                  <div className="px-8 pt-6 pb-8 overflow-y-auto hide-scrollbar">
                      ...
                      <div>
                        ...
                        <p
                          className={`text-lg font-black tracking-tight mt-0.5 ${
                            isDarkMode ? "text-slate-300" : "text-[#1877F2]"
                          }`}
                        >
                          ${bill?.amount.toFixed(2)}
                        </p>
                    ...
                    <div className="relative">
                      <label
                        className={`absolute left-4 top-2 text-[9px] font-bold uppercase tracking-widest ${
                          isDarkMode ? "text-slate-500" : "text-slate-400"
                        }`}
                      >
                        Select Account
                      ...
                      <select
                        value={paymentModalConfig.accountId}
                        onChange={(e) =>
                          setPaymentModalConfig({
                            ...paymentModalConfig,
                        ...
                        className={`w-full pt-6 pb-2 px-5 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                          isDarkMode
                    ...
                    <button
                      onClick={confirmPaymentRoute}
                      className="w-full mt-6 h-14 rounded-2xl font-bold text-lg shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 bg-[#1877F2] text-white hover:bg-blue-600 shadow-blue-500/30"
                    >
                  ...
                  <button
                    onClick={() =>
                      setEditAccountBalance(
                        editAccountBalance.slice(0, -1) || "0"
                      )
                    }
                    className="absolute right-4 text-slate-400 p-2 hover:text-slate-600 transition-colors"
                  >
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (btn === "=") {
                          try {
                            const toEval = editAccountBalance
                              .replace(/×/g, "*")
                              .replace(/÷/g, "/");
                            if (/^[0-9+\-*/. ]+$/.test(toEval)) {
                              // eslint-disable-next-line no-new-func
                        ...
                        } else if (editAccountBalance === "0" && btn !== ".") {
                          setEditAccountBalance(btn);
                        } else {
                          setEditAccountBalance(editAccountBalance + btn);
                        }
                      }}
                      className={`h-14 rounded-2xl text-xl font-bold flex items-center justify-center transition-colors shadow-sm ${
                        ["÷", "×", "-", "+", "="].includes(btn)
            ...
            >
              ...
              <div className="px-8 pt-6 pb-12 overflow-y-auto hide-scrollbar">
                ...
                <div className="space-y-4">
                  <input
                    type="text"
                    placeholder="Account Name (e.g., Citi Double Cash)"
                    value={newAccName}
                    onChange={(e) => setNewAccName(e.target.value)}
                    className={`w-full py-4 px-5 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                      isDarkMode
                  ...
                  <input
                    type="text"
                    placeholder="Short Description (e.g., Joint Account)"
                    value={newAccDesc}
                ...
                <button
                  ...
                  disabled={
                    !newAccName.trim() || isNaN(parseFloat(newAccBalance))
                  }
                  className={`w-full mt-8 h-14 rounded-2xl font-bold text-lg shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${
                    !newAccName.trim() || isNaN(parseFloat(newAccBalance))
                      ? "bg-slate-300 text-slate-500 cursor-not-allowed"
                      : "bg-[#1877F2] text-white hover:bg-blue-600 shadow-blue-500/30"
                  }`}
                >
                  Create Vault
                </button>
        ...
        {isFabOpen && (
          <div className="absolute inset-0 z-[60] flex items-end">
            ...
            ></div>
            <div
              className={`w-full rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] border-t animate-slide-up relative z-10 flex flex-col max-h-[90vh] transition-colors duration-500 ${
                isDarkMode
                  ? "bg-[#1E293B] border-slate-700"
                  : "bg-white border-slate-100"
              }`}
            >
              ...
              <div
                className={`px-6 pt-6 pb-4 border-b shrink-0 overflow-hidden ${
                  isDarkMode ? "border-slate-800" : "border-slate-50"
                }`}
              >
                <div
                ...
                {fabStep === 1 && (
                  <div
                    className={`flex rounded-xl p-1 mb-6 mr-8 ${
                  ...
                  >
                    <button
                      ...
                      className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                        drawerTab === "bills"
                          ? isDarkMode
                            ? "bg-slate-700 text-[#1877F2] shadow-sm"
                ...
                <div className="text-center mb-4 relative flex justify-center items-center">
                  ...
                  {fabStep === 1 && (
                    <button
                      ...
                      className="absolute right-0 text-slate-400 p-2 hover:text-slate-600 transition-colors"
                    >
                      ⌫
                    </button>
                  )}
              ...
              <div
                className={`p-6 mt-auto rounded-b-[2.5rem] flex-1 flex flex-col ${
                  fabStep === 1 ? "justify-end" : "justify-start pt-2"
              ...
              >
                {fabStep === 1 ? (
                  <>
                    ...
                    <button
                      onClick={() => {
                        if (parseFloat(inputValue) > 0) setFabStep(2);
                      }}
                      disabled={
                        parseFloat(inputValue) <= 0 ||
                        isNaN(parseFloat(inputValue))
                      }
                      className={`w-full mt-4 shrink-0 h-14 rounded-2xl font-bold text-lg text-white shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${
                        parseFloat(inputValue) <= 0 ||
                        isNaN(parseFloat(inputValue))
                  ...
                  <div className="flex flex-col h-full animate-fade-in">
                    <div className="space-y-4 flex-1 pb-6">
                        ...
                        <>
                          ...
                          <div className="relative">
                            ...
                            <select
                              ...
                              className={`w-full pt-6 pb-2 px-5 rounded-2xl font-bold text-xl outline-none border transition-colors appearance-none ${
                                isDarkMode
                                  ? "bg-[#1E293B] border-slate-700 text-white"
                                  : "bg-white border-slate-200 text-slate-900"
                              }`}
                          ...
                          >
                            <div
                              ...
                              onClick={() =>
                                setEntryIsInstallment(!entryIsInstallment)
                              }
                            >
                              <span
                              ...
                              <div className="mt-4 space-y-3 animate-fade-in">
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEntryTotalAmount(e.target.value)
                                    }
                                    className={`w-full pt-6 pb-2 px-5 rounded-xl font-bold text-sm outline-none border transition-colors ${
                                      isDarkMode
                                        ? "bg-[#1E293B] border-slate-700 text-white"
                                        : "bg-white border-slate-200 text-slate-900"
                                    }`}
                                ...
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEntryPaidAmount(e.target.value)
                                    }
                                    className={`w-full pt-6 pb-2 px-5 rounded-xl font-bold text-sm outline-none border transition-colors ${
                                      isDarkMode
                                        ? "bg-[#1E293B] border-slate-700 text-white"
                      ...
                      {drawerTab === "income" && (
                        <>
                          <input
                            type="text"
                            placeholder="Payer / Source (e.g., Employer, Venmo)"
                            value={entryName}
                            onChange={(e) => setEntryName(e.target.value)}
                            className={`w-full py-4 px-5 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                              isDarkMode
                      ...
                      {drawerTab === "transactions" && (
                        <>
                          <input
                            type="text"
                            placeholder="Merchant Name (e.g., Target, Uber Eats)"
                            value={entryName}
                            onChange={(e) => setEntryName(e.target.value)}
                            className={`w-full py-4 px-5 rounded-2xl font-bold text-sm outline-none border transition-colors ${
                              isDarkMode
                    ...
                    <button
                      ...
                      className={`w-full mt-4 h-14 shrink-0 rounded-2xl font-bold text-lg text-white shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${
                        drawerTab === "bills"
                          ? "bg-[#1877F2] shadow-blue-500/30"
                          : drawerTab === "income"
            ...
            >
              ...
              <div className="px-8 pt-6 pb-4 overflow-y-auto hide-scrollbar">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  >
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest w-12">
                      From
                    </span>
                    <select
                ...
                <div className="text-center mb-2 flex justify-center items-center relative">
                  <span className="text-5xl font-extrabold tracking-tighter text-[#1877F2]">
                    ${transferAmount}
                  </span>
                  <button
                    onClick={() =>
                      setTransferAmount(transferAmount.slice(0, -1) || "0")
                    }
                    className="absolute right-0 text-slate-400 p-2 hover:text-slate-600 transition-colors"
                  >
                    ⌫
              ...
              >
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        } else if (transferAmount === "0" && btn !== ".") {
                          setTransferAmount(btn);
                        } else {
                          setTransferAmount(transferAmount + btn);
                        }
                      }}
                      className={`h-14 rounded-2xl text-xl font-bold flex items-center justify-center transition-colors shadow-sm ${
                        ["÷", "×", "-", "+", "="].includes(btn)
                ...
                <button
                  ...
                  className={`w-full mt-4 h-14 rounded-2xl font-bold text-lg shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${
                    !transferFrom ||
                    !transferTo ||
                    transferAmount === "0" ||
                    transferFrom === transferTo
        ...
        {isPaydaySetupOpen && (
          <div className="absolute inset-0 z-[70] flex items-end">
            <div
              className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm animate-fade-in"
              onClick={() => setIsPaydaySetupOpen(false)}
            ></div>
            <div
              className={`w-full rounded-t-[3rem] shadow-2xl animate-slide-up relative z-10 flex flex-col max-h-[90vh] transition-colors duration-500 ${
                isDarkMode
            ...
            >
              ...
              <div className="px-8 pt-6 pb-4 overflow-y-auto hide-scrollbar">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-8">
                  Set your expected dates & income
                </p>
                <div className="space-y-4">
                  {[
                    "Payday 1",
                    ...
                    >
                      ...
                      <div className="flex gap-3">
                        <div className="flex-1">
                          ...
                          <input
                            type="date"
                            value={editPaydayConfig[pd]?.date || ""}
                            onChange={(e) =>
                              setEditPaydayConfig({
                                ...editPaydayConfig,
                                [pd]: {
                            ...
                            className={`w-full py-3 px-3 rounded-xl font-bold text-xs outline-none border transition-colors ${
                              isDarkMode
                                ? "bg-[#1E293B] border-slate-700 text-white"
                                : "bg-white border-slate-200 text-slate-900"
                            }`}
                        ...
                        <div className="flex-1">
                          ...
                          <input
                            ...
                            className={`w-full py-3 px-3 rounded-xl font-bold text-xs outline-none border transition-colors ${
                              isDarkMode
                                ? "bg-[#1E293B] border-slate-700 text-white placeholder-slate-600"
                                : "bg-white border-slate-200 text-slate-900 placeholder-slate-400"
                            }`}
                          />
                        </div>
              ...
              >
                <button
                  ...
                  className="w-full h-14 rounded-2xl font-bold text-lg shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 bg-[#1877F2] text-white hover:bg-blue-600 shadow-blue-500/30"
                >
                  Save Configuration
                </button>
              </div>
        ...
        {isNotificationsOpen && (
          <div className="absolute inset-0 z-[70] flex items-end">
            ...
            ></div>
            <div
              className={`w-full rounded-t-[3rem] shadow-2xl animate-slide-up relative z-10 flex flex-col transition-colors duration-500 max-h-[85vh] min-h-[50vh] ${
                isDarkMode
                  ? "bg-[#1E293B] border-slate-700"
                  : "bg-white border-slate-100"
              }`}
        ...
        {selectedTodo && (
          <div className="absolute inset-0 z-[70] flex items-end">
            ...
            ></div>
            <div
              className={`w-full rounded-t-[3rem] shadow-2xl animate-slide-up relative z-10 flex flex-col p-8 pb-12 transition-colors duration-500 max-h-[90vh] ${
                isDarkMode
                  ? "bg-[#1E293B] border-t border-slate-700"
                  : "bg-white border-t border-slate-100"
              }`}
            >
              <button
              ...
              <div className="mb-8 overflow-y-auto hide-scrollbar">
                ...
                <h2
                  className={`text-xl font-black leading-tight mt-2 ${
                    isDarkMode ? "text-white" : "text-slate-900"
                  }`}
                >
                  {selectedTodo.text}
              ...
              <div className="flex gap-4 mt-auto shrink-0">
                <button
                  onClick={() => {
                    toggleTodoStatus(selectedTodo.id);
                    setSelectedTodo(null);
                  }}
                  className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all ${
        ...
        <div className="absolute bottom-24 right-6 z-40">
          <button
            ...
            className={`w-14 h-14 rounded-full flex items-center justify-center text-white bg-[#1877F2] shadow-[0_12px_24px_rgba(24,119,242,0.4)] border-4 ${
        ...
        <div
          className={`absolute bottom-0 left-0 w-full backdrop-blur-md border-t px-2 pt-3 pb-6 flex justify-between items-center z-40 ${
            isDarkMode
              ? "bg-[#1E293B]/95 border-slate-800"
            ...
            >
              <tab.icon
                ...
                className={`transition-all duration-300 ${
                  activeTab === tab.id
                    ? "text-[#1877F2] transform -translate-y-1"
                    : isDarkMode
                    ? "text-slate-500"
                    : "text-slate-400"
                }`}
