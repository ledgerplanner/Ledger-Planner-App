...
export default function App() {
  ...
  // ==========================================
  // VIEW 1: LOGIN SCREEN (WITH GOOGLE & SCROLL FIX)
  // ==========================================
  if (isAuthLoading) {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex justify-center font-sans">
        <div className="w-full max-w-md bg-white min-h-screen relative shadow-2xl flex flex-col px-8 pt-12 pb-10 overflow-y-auto hide-scrollbar">
          <div className="flex flex-col items-center mb-8 shrink-0 mt-4">
            ...
            <p className="text-sm font-bold text-slate-400 mt-2 tracking-wide uppercase">{isLoginMode ? "Secure Entrance" : "Create Account"}</p>
          </div>
          
          <form onSubmit={handleAuthSubmit} className="space-y-4 flex-1 flex flex-col justify-center">
            {authError && <div className="bg-red-50 border border-red-200 text-red-600 text-xs font-bold p-3 rounded-xl flex items-center gap-2 shrink-0"><AlertCircle size={16} /> {authError}</div>}
            ...
            <button type="submit" disabled={!email || !password} className={`w-full mt-6 py-4 shrink-0 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${!email || !password ? "bg-slate-200 text-slate-400 cursor-not-allowed" : "bg-[#1877F2] text-white shadow-[0_8px_20px_rgba(24,119,242,0.3)] hover:scale-[0.98]"}`}>
            ...
            <button type="button" onClick={handleGoogleLogin} className="w-full py-3.5 shrink-0 rounded-2xl font-bold text-sm text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-[0.98]">
            ...
            <div className="text-center mt-6 shrink-0 pb-4">
