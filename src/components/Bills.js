...
export default function Bills({
  ...
  const remainingBurn = bills.filter(b => !b.isPaid).reduce((sum, b) => sum + (Number(b.amount) || 0), 0);
  ...
  // Math for the 12-segment ring
  const radius = 40;
  const circumference = 2 * Math.PI * radius;
  const segmentLength = circumference / 12;
  const gap = 3; 

  const graphicContent = (
    <div className="flex flex-col relative z-10 mb-2 w-full">
      <div className={`relative pt-10 pb-6 px-6 rounded-[2rem] border flex flex-col w-full transform transition-all duration-700 ease-out ${isMounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"} ${isDarkMode ? "bg-gradient-to-br from-blue-900/40 via-slate-800 via-25% to-slate-800 border-slate-700/50 border-t-slate-600/40 shadow-[0_12px_30px_rgba(0,0,0,0.5)]" : "bg-gradient-to-br from-blue-50/50 via-white via-25% to-slate-50 border-slate-200/60 border-t-white shadow-[inset_0_2px_3px_rgba(255,255,255,1),0_12px_24px_rgba(24,119,242,0.10),0_4px_12px_rgba(0,0,0,0.01)]"}`}>
          
        <div className="absolute top-4 left-0 w-full flex justify-center pointer-events-none">
          <span className={`text-[10px] font-black uppercase tracking-widest ${isDarkMode ? "text-white" : "text-slate-500"}`}>
            Command Center {currentYear}
          </span>
        </div>
          ...
          <div className="relative w-28 h-28 flex-shrink-0">
            <svg className="w-full h-full transform -rotate-90 drop-shadow-xl" viewBox="0 0 100 100">
              {Array.from({ length: 12 }).map((_, i) => {
                ...
                let strokeColor = isDarkMode ? "#1E293B" : "rgba(226, 232, 240, 0.9)";
                let opacity = 1;
                let strokeClassName = "transition-all duration-1000 ease-out";

                if (i < currentMonthIndex) {
                  strokeColor = "#10B981"; // Settled past months
                } else if (i === currentMonthIndex) {
                  strokeColor = signatureColor; // Active month
                  strokeClassName += " animate-pulse";
                } else {
                  opacity = isDarkMode ? 0.3 : 0.6; // Future months ghost outline
                }
            ...
            <div className={`absolute inset-0 flex flex-col items-center justify-center transform transition-all duration-700 delay-300 ease-out p-1 text-center ${isMounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
              ...
              <span className="text-[7px] font-black uppercase tracking-widest text-slate-400 mt-0.5">SETTLED</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center w-full">
            <div className="flex justify-between items-end mb-3">
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">YTD Settled</span>
                <span className="text-xl min-[380px]:text-2xl font-black tracking-tighter text-[#10B981] leading-none">
                  ${globalTotalPaid.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Remaining Burn</span>
                <span className="text-base min-[380px]:text-lg font-black tracking-tighter leading-none" style={{ color: signatureColor }}>
                  ${remainingBurn.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
            </div>

            <div className={`w-full py-2.5 px-3 rounded-xl border flex items-center justify-between shadow-sm transition-all ${urgentTotal > 0 ? (isDarkMode ? 'bg-red-900/20 border-red-900/50' : 'bg-red-50 border-red-200') : (isDarkMode ? 'bg-[#10B981]/10 border-[#10B981]/20' : 'bg-emerald-50 border-emerald-200')}`}>
               <span className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${urgentTotal > 0 ? 'text-red-500' : 'text-[#10B981]'}`}>
                 ...
                 {urgentTotal > 0 ? 'URGENT DUE NOW' : 'BOARD CLEAR'}
               </span>
               <span className={`text-sm min-[380px]:text-base font-black leading-none ${urgentTotal > 0 ? 'text-red-500' : 'text-[#10B981]'}`}>
        ...
        <div className="flex gap-4 pr-6 pb-2 min-h-[170px] snap-x snap-mandatory">
          {monthsData.map((m) => {
            ...
            if (isPastMonth) {
              ...
              if (isSelected) {
                cardBackgroundClass = isDarkMode ? "bg-blue-900/20 shadow-md scale-[1.01]" : "bg-blue-50/80 shadow-[0_4px_20px_rgba(24,119,242,0.15)] scale-[1.01]";
              ...
              if (isSelected) {
                ...
                cardBackgroundClass = isDarkMode ? "bg-[#1E293B] shadow-[0_0_35px_rgba(24,119,242,0.4)] border-2 scale-[1.01]" : "bg-white shadow-[0_0_30px_rgba(24,119,242,0.2)] border-2 scale-[1.01]";
                buttonText = "VIEW DETAILS";
                buttonStyleClass = isDarkMode ? "bg-slate-800/80 hover:bg-slate-800 text-slate-400 border border-slate-700 font-bold shadow-sm" : "bg-slate-50 hover:bg-slate-100 text-slate-500 border border-slate-200 font-bold shadow-sm";
              }
            } else {
            ...
            return (
              <div
                ...
                style={{ borderColor: (!isPastMonth && isCurrentMonth && !isSelected) ? signatureColor : (isSelected ? signatureColor : undefined) }}
              >
          ...
          <div
            ...
            className={`shrink-0 w-52 p-5 rounded-[1.75rem] border cursor-pointer active:scale-[0.95] snap-center transition-all flex flex-col justify-between h-44 ${selectedMonth === 12 ? (isDarkMode ? "bg-blue-900/20 shadow-md scale-[1.01]" : "bg-blue-50/80 shadow-[0_4px_20px_rgba(24,119,242,0.15)] scale-[1.01]") : (isDarkMode ? "bg-[#1E293B] border-slate-700 shadow-md" : "bg-white/90 backdrop-blur-sm border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)]")}`}
